...
while gridScene.IsRunning
    ...
    if any(motion(1) < (xlimitsScene(1) + 10)) 
        xBounce = 1;
    elseif any(motion(1) > (xlimitsScene(2) - 10))
        xBounce = -1;
    end

    if any(motion(2) < (ylimitsScene(1) + 10)) 
        yBounce = 1;
    elseif any(motion(2) > (ylimitsScene(2) - 10))
        yBounce = -1;
    end

    if xBounce ~= 0 || yBounce ~= 0
        disp('yBounce');
        disp(yBounce);
        disp('xBounce');
        disp(yBounce);

        motion(4) = rand * maxVelocity * xBounce; % Random new x velocity
        disp('new motion(4)');
        disp(motion(4));

        motion(5) = sqrt(maxVelocity^(2) - motion(4)^(2)) * yBounce; % Random new y velocity
        disp('BOUNCE');
    end
